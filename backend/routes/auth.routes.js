...
import jwt from "jsonwebtoken";
import { requireAuth } from "../middleware/auth.js"; // your JWT verifier
import userController from "../controllers/users-controllers.js"; // for /me, if needed
...
router.post("/login", async (req, res) => {
  ...
  const { email, password, mdp } = req.body || {};
  const pwd = password ?? mdp;
  try {
    // validate user/password here (DB check). Example:
    const user = await userController.Login(email, password); // or your own logic returning a user object
...
router.get("/me", requireAuth, async (req, res) => {
  try {
    // Either return what's in the token…
    // res.json({ user: { id: req.user.sub, email: req.user.email, role: req.user.role, is_admin: req.user.is_admin }});

    // …or load a fresh user from DB if you prefer:
    const user = await userController.GetById(req.user.sub || req.user.id);
    res.json({
      user: {
        id: user.id,
        email: user.email,
    ...
    res.status(500).json({ message: "Cannot fetch user", error: e.message });
  }
});
